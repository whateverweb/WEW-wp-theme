...
function getStatistics(userEmail,appKey){
	...
	currentRequestLink=WhateverConfig.baseURL + 'core/req/acc/' + userEmail + '/'+sessionToken;
	if(appKey){
		//appKey='e2db877b-2b78-4653-840f-3e831987cc69';
		currentRequestLink=WhateverConfig.baseURL + 'core/req/app/' + appKey + '/'+sessionToken;
	}
	
	
	feedingData=[['Week', 'requests']];
...
function getTimeFrameCallback(resultJSON) {
	...
	if (resultJSON.statusCode != null) {
		...
		for(var i=0;i<intervalsNumber;i++){
			feedingData.push([labelsString[i],resultJSON[i].totalRequestsPerQueryInterval]);
		}
